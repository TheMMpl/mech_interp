...
noise = torch.randn_like(steering_vector)
        
# Calculate norms
noise_norm = noise.norm()
ref_norm = steering_vector.norm()
...
def steering_hook(module, input, output):
    ...
    if isinstance(output, tuple):
        ...
        if steer_all_tokens:
            ...
            modified_states[:, injection_pos, :] = modified_states[:, injection_pos, :] + steering_vector * 6
        ...
        if steer_all_tokens:
            ...
            modified_states[:, injection_pos, :] = modified_states[:, injection_pos, :] + steering_vector
