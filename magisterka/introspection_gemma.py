...
Trial 1: Do you detect an injected thought? Answer only by 'Yes' or 'No'."""}]
    ...
    def _detect_architecture(self):
        ...
        def _is_hookable_layer_list(maybe_layers) -> bool:
            try:
                return (
                    maybe_layers is not None
                    and hasattr(maybe_layers, "__len__")
                    and len(maybe_layers) > 0
                    and hasattr(maybe_layers[0], "register_forward_hook")
                )
            except Exception:
                return False

        def _find_layer_list_fallback(root, max_depth: int = 6):
            """Breadth-first search for a plausible decoder block list."""
            ...
            while queue:
                obj, obj_path, depth = queue.popleft()
                obj_id = id(obj)
                ...
                # Direct hit: object itself is a candidate layer list
                if _is_hookable_layer_list(obj):
                    return obj, (obj_path if obj_path else "<root>")

                # Explore attributes conservatively to avoid huge traversals
                try:
                ...
                for name in attr_names:
                    ...
                    # Skip simple scalars/containers to keep search focused
                    if isinstance(child, (str, bytes, int, float, bool, tuple, list, dict, set)):
                        continue

                    child_path = f"{obj_path}.{name}" if obj_path else name

                    if _is_hookable_layer_list(child):
                        return child, child_path

                    # Continue traversal through module-like objects only
                    if hasattr(child, "__dict__"):
                        queue.append((child, child_path, depth + 1))

            return None, None

        # Common layer layouts across decoder-only families and newer Gemma variants.
        # Keep highest-probability paths first to minimize overhead.
        candidates = [
            ("model.layers", "transformer"),
            ("model.model.layers", "transformer"),
            ("model.language_model.layers", "transformer"),
            ("model.language_model.model.layers", "transformer"),
            ("model.model.language_model.layers", "transformer"),
            ("model.model.language_model.model.layers", "transformer"),
            ("model.decoder.layers", "transformer"),
            ("model.model.decoder.layers", "transformer"),
            ("model.transformer.layers", "transformer"),
            ("model.model.transformer.layers", "transformer"),
            ("language_model.model.layers", "transformer"),
            ("language_model.layers", "transformer"),
            ("transformer.h", "gpt"),
            ("gpt_neox.layers", "gpt-neox"),
        ]

        selected_path = None
        for attr_path, arch_name in candidates:
            maybe_layers = _resolve_attr_path(self.model, attr_path)
            if maybe_layers is None:
                continue
            if _is_hookable_layer_list(maybe_layers):
                self.layer_modules = maybe_layers
                self.architecture = arch_name
                selected_path = attr_path
                break

        # Final fallback for unfamiliar wrapper nesting (e.g., new Gemma variants)
        if selected_path is None:
            fallback_layers, fallback_path = _find_layer_list_fallback(self.model)
            if fallback_layers is not None:
                self.layer_modules = fallback_layers
                self.architecture = "transformer"
                selected_path = fallback_path

        if selected_path is None:
            available_top = [name for name in ("model", "language_model", "transformer", "gpt_neox") if hasattr(self.model, name)]
            raise ValueError(
                f"Unsupported model architecture for {self.model_name}. "
                f"Tried paths: {[p for p, _ in candidates]}. "
                f"Top-level modules found: {available_top}"
            )

        if self.verbose:
            print(f"Detected architecture: {self.architecture}")
            print(f"Layer path: {selected_path}")
            print(f"Number of layers: {len(self.layer_modules)}")

    def _setup_yes_no_tokens(self):
        """Setup and cache Yes/No token IDs."""
        # Some tokenizers (Llama-3.3, Gemma family) use bare tokens without leading space.
        model_name_lower = self.model_name.lower()
        if self.model_name == "meta-llama/Llama-3.3-70B-Instruct" or "gemma" in model_name_lower:
            yes_token = 'Yes'
            no_token = 'No'
        else:
            yes_token = ' Yes'
    ...
    def _load_refusal_direction(self, vector_path: str) -> torch.Tensor:
        """Load and normalize refusal direction from a .pt file."""
        if vector_path is None:
            raise ValueError("A refusal direction path is required for ablation mode")

        loaded = torch.load(vector_path, map_location='cpu')

        # Accept a plain tensor or common dict wrappers.
        if isinstance(loaded, dict):
            if 'direction' in loaded:
                direction = loaded['direction']
            elif len(loaded) == 1:
                direction = next(iter(loaded.values()))
            else:
                raise ValueError(
                    f"Refusal direction file must contain a tensor or a dict with key 'direction'. Got keys: {list(loaded.keys())}"
                )
        else:
            direction = loaded
        ...
        try:
            direction = direction.to(next(self.model.parameters()).dtype)
        except Exception:
            pass

        direction_norm = direction.norm()
        if direction_norm.item() == 0:
            raise ValueError("Loaded refusal direction has zero norm and cannot be used for ablation")

        return direction / direction_norm

    def _project_onto_direction(self, hidden_states: torch.Tensor, direction: torch.Tensor) -> torch.Tensor:
        """Project hidden states onto a direction for projection-based ablation."""
        direction_broadcast = direction.view(1, 1, -1)
        coeffs = (hidden_states * direction_broadcast).sum(dim=-1, keepdim=True)
        return coeffs * direction_broadcast

    def _apply_refusal_ablation(
        self,
        ...
        """Apply projection ablation to all tokens or a single token position."""
        modified_states = hidden_states.clone()
        if steer_all_tokens:
            projection = self._project_onto_direction(modified_states, direction)
            return modified_states - (ablation_strength * projection)

        seq_len = modified_states.shape[1]
        token_idx = token_pos if token_pos >= 0 else seq_len + token_pos
        if token_idx < 0 or token_idx >= seq_len:
            return modified_states

        token_slice = modified_states[:, token_idx:token_idx + 1, :]
        projection = self._project_onto_direction(token_slice, direction)
        modified_states[:, token_idx:token_idx + 1, :] = token_slice - (ablation_strength * projection)
        return modified_states

    def _resolve_ablation_layer_indices(self, steering_layer_idx: int, ablation_scope: str) -> List[int]:
        """Resolve which layers receive ablation hooks."""
        num_layers = len(self.layer_modules)
        if steering_layer_idx < 0 or steering_layer_idx >= num_layers:
            raise ValueError(f"Invalid steering layer index: {steering_layer_idx}")

        if ablation_scope == "all_layers":
            return list(range(num_layers))
        if ablation_scope == "from_layer_onward":
            return list(range(steering_layer_idx, num_layers))
        if ablation_scope == "steering_layer":
            return [steering_layer_idx]

        raise ValueError(
            f"Unsupported ablation_scope '{ablation_scope}'. Expected one of: all_layers, from_layer_onward, steering_layer"
        )

    def _register_refusal_ablation_hooks(
        self,
        ...
        """Register identical ablation hooks on a set of layers and return handles."""
        handles = []

        def ablation_hook(module, input, output):
            if isinstance(output, tuple):
                hidden_states = output[0]
                modified_states = self._apply_refusal_ablation(
                    hidden_states, direction, ablation_strength, token_pos, steer_all_tokens
                )
                return (modified_states,) + output[1:]
            hidden_states = output
            return self._apply_refusal_ablation(
                hidden_states, direction, ablation_strength, token_pos, steer_all_tokens
            )

        for idx in layer_indices:
            handles.append(self.layer_modules[idx].register_forward_hook(ablation_hook))
        return handles

    def run_with_refusal_ablation(
        self,
        layer_idx: int,
        ...
        token_pos: int = -1,
        steer_all_tokens: bool = False,
        ablation_scope: str = "steering_layer",
    ) -> Tuple[float, float]:
        """Run logits test with projection ablation along a refusal direction.

        The ablation removes scale * projection(hidden, direction) at the chosen
        layer output during inference. Scale 1.0 removes the component, while
        larger values over-remove and can flip the component.
        """
        direction = self._load_refusal_direction(refusal_direction_path)

        if self.verbose:
            steer_mode = "all tokens" if steer_all_tokens else f"token pos {token_pos}"
            print(f"\n=== Layer {layer_idx}, Refusal Ablation, Strength {ablation_strength}, Scope {ablation_scope}, Steer {steer_mode} ===")
            print(f"  Direction path: {refusal_direction_path}")

        layer_indices = self._resolve_ablation_layer_indices(layer_idx, ablation_scope)
        hook_handles = self._register_refusal_ablation_hooks(
    ...
    def run_with_steering_and_refusal_ablation(
        self,
        layer_idx: int,
        magnitude: float,
        refusal_direction_path: str,
        ...
        """Run logits with steering and refusal ablation active in the same forward pass."""
        direction = self._load_refusal_direction(refusal_direction_path)
        layer_indices = self._resolve_ablation_layer_indices(layer_idx, ablation_scope)
        ...
        if self.verbose:
            steer_mode = "all tokens" if steer_all_tokens else f"token pos {token_pos}"
            print(
                f"\n=== Layer {layer_idx}, Steering {magnitude} + Refusal Ablation {ablation_strength}, Scope {ablation_scope}, Steer {steer_mode} ==="
            )
            print(f"  Direction path: {refusal_direction_path}")
    ...
    def run_scale_sweep(
        ...
        concept_key: Optional[str] = None,
        vector_path: Optional[str] = None,
        append_words: Optional[List[str]] = None,
        ablate_refusal: bool = False,
        refusal_direction_path: Optional[str] = None,
        ablation_scope: str = "all_layers",
    ):
        ...
        Args:
            ...
            steer_all_tokens: If True, apply steering to all token positions
            concept_key: If provided, use stored concept vector instead of contrastive prompts
            vector_path: Path to a saved vector mapping to load concept vectors
            append_words: Optional list of words to append progressively as prefixes
            ablate_refusal: If True, apply refusal ablation (strength=1.0) together with steering
            refusal_direction_path: Path to .pt file containing refusal direction tensor
            ablation_scope: Which layers get ablation: all_layers, from_layer_onward, steering_layer
        ...
        if self.verbose:
            ...
            if ablate_refusal:
                print(
                    f"Ablation mode: steering + refusal ablation (strength=1.0, scope={ablation_scope}) from '{refusal_direction_path}'"
                )
        ...
        for scale_idx, scale in enumerate(scales):
            ...
            # Special case: scale=0 is baseline only when ablation mode is disabled.
            if scale == 0 and not ablate_refusal:
        ...
        append_results = None
        if plot:
            # If requested, compute data points for progressively appended prefixes
            if append_words:
                append_results = []
                if self.verbose:
                    print(f"Computing append-prefix points for '{' '.join(append_words)}' (no injection)")

                for i in range(1, len(append_words) + 1):
                    prefix = ' '.join(append_words[:i])
                    try:
                        # Introspection prompt with appended prefix
                        intro_msgs = copy.deepcopy(self.introspection_question)
                        if isinstance(intro_msgs[-1], dict) and 'content' in intro_msgs[-1]:
                            intro_msgs[-1]['content'] = intro_msgs[-1]['content'].rstrip() + ' ' + prefix
                        else:
                            intro_msgs = self.format_prompt(self.introspection_question) + ' ' + prefix

                        if isinstance(intro_msgs, list):
                            prompt_intro = self.format_prompt(intro_msgs)
                        else:
                            prompt_intro = intro_msgs

                        inputs_intro = self.tokenizer(prompt_intro, return_tensors="pt").to(self.device)
                        _, append_intro = self.get_top_logits(inputs_intro)

                        # Control prompt with appended prefix
                        control_msgs = copy.deepcopy(self.control_question)
                        if isinstance(control_msgs[-1], dict) and 'content' in control_msgs[-1]:
                            control_msgs[-1]['content'] = control_msgs[-1]['content'].rstrip() + ' ' + prefix
                        else:
                            control_msgs = self.format_prompt(self.control_question) + ' ' + prefix

                        if isinstance(control_msgs, list):
                            prompt_control = self.format_prompt(control_msgs)
                        else:
                            prompt_control = control_msgs

                        inputs_control = self.tokenizer(prompt_control, return_tensors="pt").to(self.device)
                        _, append_control = self.get_top_logits(inputs_control)

                        append_results.append((prefix, float(append_intro), float(append_control)))
                        if self.verbose:
                            print(f"  Append prefix='{prefix}' - Introspection: {append_intro:+.3f}, Control: {append_control:+.3f}")
                    except Exception as e:
                        print(f"Warning: failed to compute append-prefix '{prefix}': {e}")
            ...
            self._plot_scale_effects(
                ...
                mode_label=(
                    f"Steering sweep + refusal-direction ablation=1.0, scope={ablation_scope}, steering layer={layer_idx} ({refusal_direction_path})"
                    if ablate_refusal else None
                )
            )
    ...
    def _plot_scale_effects(
        ...
        baseline_control: float,
        layer_idx: int,
        contrastive_prompts: Optional[Tuple[str, str]],
        concept_key: Optional[str] = None,
        append_results: Optional[List[Tuple[str, float, float]]] = None,
        ...
        # If append-results were provided, add them as separate points to the right of the scales
        if append_results:
            x_base = max(scales) if len(scales) > 0 else 0.0
            offset_unit = 0.05 * max(1.0, x_base)
            for idx, (word, a_intro, a_control) in enumerate(append_results):
                x_pos = x_base + (idx + 1) * offset_unit
                plt.plot([x_pos], [a_intro], marker='D', markersize=9, color='purple', label=(f"Append '{word}' (introspection)" if idx == 0 else None))
                plt.plot([x_pos], [a_control], marker='X', markersize=9, color='brown', label=(f"Append '{word}' (control)" if idx == 0 else None))
                # Annotate with short label
                ax.text(x_pos, a_intro, f" {word}", fontsize=8, color='purple', va='bottom', rotation=30)
                ax.text(x_pos, a_control, f" {word}", fontsize=8, color='brown', va='top', rotation=30)
        ...
        if mode_label is not None:
            ...
            steering_info = f'Steering at layer {layer_idx}'
        ...
        model_short = self.model_name.split("/")[-1].replace(".", "_")
        if mode_label is not None:
            filename = f"introspection_{model_short}_ablation_layer{layer_idx}_scale_sweep.png"
        elif concept_key is not None:
